...
"[Learning-Programming-Massively-Parallel-Processors/src/pmpp/models/attention.py](https://github.com/jamesnulliu/Learning-Programming-Massively-Parallel-Processors/blob/cf690614d004aa647aefccb8db3eac83255cb99e/src/pmpp/models/attention.py)"
...
1. **Input New Token**: Take the generated token from last round as input sequence, obtain `Q_new`, `K_new` and `V_new` through linear transformation.
4. **Update KV Cache**: `K_new` and `V_new` are added to the end of `K_cache` and `V_cache`, making them a pair of `(kv_len + 1, embed_dim)` vectors.
...
5. **Output**: The output after attention calculation has shape `(1, embed_dim)`, which is the newly generated token.
...
The general structure of the Paged Attention kernel is as follows:
